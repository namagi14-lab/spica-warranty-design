...
| ミラーDB | `prod_process_execution_db`（MySQL）—ダッシュボード用 |
...
- **`MySqlConnection`**: MySQL（`prod_process_execution_db`）—ミラー書き込み専用
...
| `OperatorMsg` | List\<string\> | YES | タブレットへ表示する作業指示メッセージリスト（JSON格納） |
...
## 4. MySQL (`prod_process_execution_db`) — ミラーDB

`ProdProcessDbContext`（EF Core / MySQL）で管理。  
CarrotRapeはバッチファイル（`.bat`）経由でここに書き込む（直接EFで書いてはいない）。
...
DashboardProgram（C0L-0164）はこのDBをREAD ONLYで参照する。CarrotRapeが書く。
...
4. バッチがMySQLへのINSERT/UPDATEを実行する（バッチ内部の実装はCarrotRapeリポジトリ外）
...
MiniPC・タブレット向けのメイン工程制御API。

| メソッド | エンドポイント | 用途 |
|---------|--------------|------|
| POST | `/api/Session` | セッション開始（MiniPCが`RaspiNo`等を送信） |
| GET | `/api/Session/{id}` | 特定セッションの状態確認 |
| GET | `/api/Session/{id}/Next` | 次の工程JSON（Jig_Process）を取得してSessionを更新 |
| GET | `/api/Session/{id}/Next/Json` | 次の工程JSONのみ取得 |
| GET | `/api/Session/{id}/Retry` | 現在の工程JSONを再取得（リトライ用） |
| GET | `/api/Session/{id}/Previous` | 一つ前の工程JSONを取得 |
| GET | `/api/Session/{id}/SetStep/{seq}` | 指定SequenceNoの工程JSONを取得 |
| PUT | `/api/Session/{id}/State` | セッション状態（StateType）を更新 |
| PUT | `/api/Session/{id}/ReturnData` | Raspiからの返却データ（JSON結果）を登録 |
| GET | `/api/Session/PutResultToId/{id}/{allResult}` | 指定IDのセッションにOK/NG結果を登録 |
| GET | `/api/Session/PutResultToLastLine/{serialNo}/{sequenceType}/{allResult}` | シリアル番号で最新RUNNINGセッションに結果登録 |
| GET | `/api/Session/PutGetWaitTablet/{id}/{elapsedTime}` | タブレット向け作業指示送信＆完了待ち（C#ライブラリ用） |
| GET | `/api/Session/PutGetWaitTabletSwagger/{id}/{elapsedTime}` | 同上（Swagger用） |
| GET | `/api/Session/GetTablet/{tabletNo}` | タブレット向け作業待ちセッション一覧取得 |
| GET | `/api/Session/GetTabletById/{tabletNo}/{id}` | 特定IDのセッションをタブレットにLOCK |
| PUT | `/api/Session/PutTabletExeStatus/{id}/ExeStatusType` | タブレットからOK/NG結果を返す（C#ライブラリ用） |
| PUT | `/api/Session/PutTabletExeStatusSwagger/{id}/ExeStatusType` | 同上（Swagger用） |
| PUT | `/api/Session/PutUnlockAllForTablet/{tabletNo}` | タブレットの全LOCKを解除 |
| PUT | `/api/Session/PutUnlockForTabletById/{id}` | 特定セッションのLOCKを解除 |
| PUT | `/api/Session/UpdateSessionHistory` | SessionHistoryを更新（＋MySQLミラー） |
| DELETE | `/api/Session/DeleteSession/{id}` | セッション削除 |
| GET | `/api/Session/list` | 実行中セッション一覧（看板用） |
| GET | `/api/Session/listAll` | 全セッション一覧 |
| GET | `/api/Session/listAll_History` | 履歴テーブル一覧 |
...
### 6-1. MiniPC（通常工程）のフロー

```
MiniPC                         CarrotRape                     SQL Server
  │                               │                              │
  │ POST /api/RaspiDatas/{raspiNo}│                              │
  │──────────────────────────────>│ UPSERT RaspiData             │
  │                               │─────────────────────────────>│
  │                               │                              │
  │ POST /api/Session             │                              │
  │  { RaspiNo, SequenceTYPE, SerialNo="XXXXXXXXXX" }           │
  │──────────────────────────────>│ INSERT Session               │
  │                               │  (State=Initialize)          │
  │<── { id: 123 }                │─────────────────────────────>│
  │                               │                              │
  │ GET /api/Session/123/Next     │                              │
  │──────────────────────────────>│ SELECT Jig_Process           │
  │                               │  WHERE MODELTYPE=xxx         │
  │                               │  AND SequenceNo > 0          │
  │                               │─────────────────────────────>│
  │<── { JsonData, SequenceNo }   │ UPDATE Session               │
  │                               │  (State=Raspi_wait/...) ───>│
  │ [治具JSONを実行]              │                              │
  │                               │                              │
  │ PUT /api/Session/123/ReturnData│                             │
  │  { ALLResult: "OK" }          │                              │
  │──────────────────────────────>│ UPDATE Session               │
  │                               │  (ALLResult=OK) ───────────>│
  │                               │                              │
  │ GET /api/Session/PutResultToId/123/OK                        │
  │──────────────────────────────>│ UPDATE Session               │
  │                               │  (ALLResult=OK)              │
...
### 6-2. タブレット作業指示フロー（PutGetWaitTablet方式）

```
MiniPC                    CarrotRape                 タブレット
  │                          │                          │
  │ GET PutGetWaitTablet/123/0│                          │
  │  ?operatorMsg=["確認してください"]                  │
  │─────────────────────────>│ Session.RaspiExeStatus=WAIT
  │                          │ Session.OperatorMsg=[...]│
  │                          │                          │
  │                          │ ←タブレットがGetTablet等でWAITを検知
  │                          │                          │
  │ GET PutGetWaitTablet/123/1│                         │ PUT PutTabletExeStatus/123/OK
  │─────────────────────────>│                          │───────────────────────────>│
  │<── WAIT                  │ Session.RaspiExeStatus=OK│
  │                          │<─────────────────────────│
  │ GET PutGetWaitTablet/123/2│                         │
  │─────────────────────────>│                          │
  │<── OK ←─────────────────│ RaspiExeStatus==OK       │
  │                          │                          │
  │ [次ステップへ進む]       │                          │
```
...
- [12_host_pc_app_pivot.md](12_host_pc_app_pivot.md) — アーキテクチャ移行の全体方針
- [08_image_inspection_db.md](08_image_inspection_db.md) — `host_pc_db`の旧DB仕様
- [02_db_schema.md](02_db_schema.md) — `prod_process_execution_db`（MySQL）のスキーマ
