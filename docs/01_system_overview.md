...
### C0L-0160 WorkInstructionApp（HostPC上）
...
- プリンター本体にKCFGコマンドを送信
- WorkInstructionApp のAPIを呼び出してトランザクションを登録・更新
- 工程JSONに従ってステップを順次実行

### C0L-0162 画像検査Program（ライン内・画像検査PC上）

- 実機スキャナを使った画像検査
- 検査結果・作業指示トランザクションを **`image_inspection_db`（HostPC上の画像検査専用DB）へ直接書き込む**
- WorkInstructionApp の WebAPI は使用しない（暫定構成）
- 作業指示が必要なステップでは `image_inspection_db` に作業指示レコードを INSERT し、WorkInstructionApp 経由でタブレットに表示させる
- オペレーターの OK/NG 応答は WorkInstructionApp が `image_inspection_db` を更新することで通知される

> **暫定構成の背景**: C0L-0162 の担当開発者が現行システムの API 連携対応が困難なため、  
> 既存のシステム構成を流用する形で `image_inspection_db` を仲介する暫定対応を採用。  
> **将来的には C0L-0161 と同様の API 連携方式に統一する。**

### C0L-0163 作業指示Program（ライン内・タブレット上）

- ライン内のタブレットで動作し、オペレーターに作業指示を表示するプログラム
- WorkInstructionApp から SignalR で作業指示を受信して画面表示する
- オペレーターの OK/NG 入力を WorkInstructionApp へ送信する
...
- WorkInstructionApp を経由せず DB から直接データ参照する
- SignalRでWorkInstructionAppから**更新通知**を受信し、通知をトリガーにDBへ再クエリして画面を更新
