...
- HostPCProgram の WebAPI は使用しない（暫定構成）
- 作業指示が必要なステップでは `image_inspection_db` に作業指示レコードを INSERT し、HostPCProgram 経由でタブレットに表示させる
- オペレーターの OK/NG 応答は HostPCProgram が `image_inspection_db` を更新することで通知される
