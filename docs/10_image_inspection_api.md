...
> **方針**: 画像検査Program（C0L-0162）は**一切変更しない**。WorkInstructionApp（C0L-0160）が DB をポーリングして変化を検知し、タブレットへ SignalR でプッシュする。
...
```
画像検査Program     image_inspection_db     WorkInstructionApp     作業指示Program
...
sequenceDiagram
    ...
    participant H  as WorkInstructionApp<br>(C0L-0160)
...
WorkInstructionApp が**読む**列・**書く**列を明示する。

| 列名 | 型 | WorkInstructionApp | 説明 |
|-----|----|--------------------|------|
...
| `WAIT` | **WorkInstructionApp** | タブレットに通知済み・応答待ち |
| `OK` | **WorkInstructionApp** | オペレーターが OK を選択 |
| `NG` | **WorkInstructionApp** | オペレーターが NG を選択 |

> `WAIT` への更新は、重複ポーリング処理防止のためのロック。`NA` → `WAIT` は **WorkInstructionApp** が行い、その後 SignalR 送信する。
...
WorkInstructionApp は主に **READ** 用途（結果確認・表示）。
...
### 3.3 参照のみテーブル（WorkInstructionApp は READ のみ）
...
| C0L-0162 が設定する値 | WorkInstructionApp の対応 |
|---------------------|--------------------------|
| `State = 'ImagePC_wait'` | 参照のみ（ログ・Dashboard 用） |
| `OperatorMsg` に `"Tablet_Interruptible"` | **この値を検知してタブレット通知のトリガーとする** |
| `TabletExeStatus = 'NA'` | `'WAIT'` → `'OK'/'NG'` に WorkInstructionApp が更新 |
| `TabletNo` に担当タブレット ID | SignalR の送信先解決に使用 |

> **制約**: WorkInstructionApp は `Session.ALLResult` や `State` を**更新しない**。  
...
- [`07_system_design.md`](07_system_design.md) — システム全体設計（WorkInstructionApp の役割）
